...
test('provisjon: 4 salg gir fortsatt 15', function () {
  assert.strictEqual(Fmt.provisjon(4).sats, 0.15);
});
test('provisjon: 5 salg gir 20', function () {
  assert.strictEqual(Fmt.provisjon(5).sats, 0.20);
});
test('provisjon: 15 salg gir 25', function () {
  assert.strictEqual(Fmt.provisjon(15).sats, 0.25);
});
...
test('provisjon: til neste fra 3 er 2', function () {
  assert.strictEqual(Fmt.provisjon(3).tilNeste, 2);
});
test('provisjon: til neste fra 5 er 10', function () {
  assert.strictEqual(Fmt.provisjon(5).tilNeste, 10);
});
...
test('provisjon: fremdrift i trinnet', function () {
  ...
  assert.strictEqual(p.trinnStorrelse, 5);
});
test('provisjon: negativt behandles som 0', function () {
  assert.strictEqual(Fmt.provisjon(-4).sats, 0.15);
});

/* --------------------------------------------- paritet med Apps Script */
// Kopi av formelen i Kode.gs hentTall(). Testen faller hvis de to trappene
// drifter fra hverandre, saa pengene selgeren ser alltid stemmer med visningen.
function gasSats(b) {
  return b >= 30 ? 0.30 : b >= 15 ? 0.25 : b >= 5 ? 0.20 : 0.15;
}
function gasTilNeste(b) {
  return b >= 30 ? 0 : b >= 15 ? 30 - b : b >= 5 ? 15 - b : 5 - b;
}
